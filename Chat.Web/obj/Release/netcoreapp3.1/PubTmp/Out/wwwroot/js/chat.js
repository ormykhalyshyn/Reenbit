$(document).ready(function () {
    ...
    //connection.on("DeleteMessFromRoom", function (room, message) {
    //    viewModel.deleteMessFromRoom(new ChatRoom(room.id, room.name));
    /*});*/
    ...
    //connection.on("removeMsgFRoom", function (room, message) {
    //    viewModel.msgDeleted(room.id, message.id);
    //});
    ...
    function AppViewModel() {
        ...
        self.deleteMessFRoom = function (id) {
            connection.invoke("DeleteMessFRoom", self.joinedRoom(), id);
        }
        self.changeMessRoom = function (id) {
            connection.invoke("ChangeMessFRoom", self.joinedRoom(), id);
        }
        ...
        self.messageHistory = function () {
            connection.invoke("GetMessageHistory", self.joinedRoom()).then(function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    self.chatMessages.push(new ChatMessage(result[i].id,result[i].content,
                        result[i].timestamp,
                        result[i].from,
                        isMine,
                        result[i].avatar))
        ...
        self.msgDeleted = function (id,idm) {
            var temp;
            ko.utils.arrayForEach(self.chatRooms(), function (room) {
                ko.utils.arrayForEach(self.message(), function (mess) {
                    if (room.id() == id && mess.id == idm) {
                        temp = mess;
                    }
                });
            });
            self.message.remove(temp);
        }
    ...
    function ChatMessage(idMsg,content, timestamp, from, isMine, avatar) {
        var self = this;
        self.id = ko.observable(idMsg);
