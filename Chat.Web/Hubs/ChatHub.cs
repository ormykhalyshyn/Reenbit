...
namespace Chat.Web.Hubs
{
    ...
    public class ChatHub : Hub
    {
        ...
        public readonly static List<MessageViewModel> _Message = new List<MessageViewModel>();
        ...
        public async Task SendPrivate(string receiverName, string message/*, string roomName*/)
        {
            if (_ConnectionsMap.TryGetValue(receiverName, out string userId))
            {
                ...
                if (!string.IsNullOrEmpty(message.Trim()))
                {
                    ...
                    {
                        Content = Regex.Replace("(Private) " + message, @"(?i)<(?!img|a|/a|/img).*?>", string.Empty),
        ...
        public async Task DeleteMessFRoom(string roomName, int idMsg)
        {
            try
            {
                // Delete from database
                var messageHistory = _context.Messages.Where(m => m.ToRoom.Name == roomName && m.Id == idMsg).FirstOrDefault();

                _context.Messages.Remove(messageHistory);
                _context.SaveChanges();

                // Delete from list
                var msgViewModel = _Message.First(r => r.Id == idMsg);
                _Message.Remove(msgViewModel);
            }
            catch (Exception)
            {
                await Clients.Caller.SendAsync("onError", "Can't delete this message. Only owner can delete this message.");
            }
        }

        public async Task ChangeMessFRoom(string roomName, int idMsg, string newMsg)
        {
            try
            {
                // Delete from database
        ...
        {
            get { return _context.Messages.Count(); }
        }
